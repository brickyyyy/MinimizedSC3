...
kmeans1=kMeans[["euclidean_laplacian_10_10"]]
kmeans1=FindSimilarities(kmeans1)
pheatmap(kmeans1)
```
...
pheatmap(cons)
```
##Check the size of the matrix after SEC 
For the Deng dataset the matrix has a size 2680x30
```{r}
library(SC3min)
dengMin = sc3min(deng, ks=10)
cons1 = metadata(dengMin)$sc3min$consensus[["10"]][["test"]]
pheatmap::pheatmap(cons1)
```
